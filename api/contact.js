...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "briankm1220@gmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "BK Roofing <leads@bkroofingllc.com>";

  const subject = `New lead from bkroofingllc.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on bkroofingllc.com`;
  const html = `
<h2>New lead from the BK Roofing site</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on bkroofingllc.com</p>`;
